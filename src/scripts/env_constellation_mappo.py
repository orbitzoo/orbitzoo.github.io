...
class ConstellationEnv(OrbitZoo):
    ...
    def observations(self):
        ...
        for i, sc in enumerate(self.dynamics.spacecrafts):
            ...
            central_state += [normalized_sma, eccentricity, normalized_fuel]

        central_state += normalized_anomalies

        return observations, central_state
...
observations, central_state = env.observations()
...
algorithm = MAPPO(
    ...
    central_state_dim=len(central_state),
...
# load model
algorithm.load(f"trained_models/constellation/mappo/model_geo_mappo")
...
for episode in range(1, episodes + 1):
    ...
    for t in range(1, steps_per_episode + 1):
        ...
        new_observations, new_central_state = env.observations()
        ...
        # save experience
        algorithm.central_states.append(torch.FloatTensor(central_state).unsqueeze(0).to(device))
