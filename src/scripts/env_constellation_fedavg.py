...
for episode in range(1, episodes + 1):
    ...
    for t in range(1, steps_per_episode + 1):
        ...
        if time_step % update_freq == 0:
            ...
            # average critics
            critics = [algorithms[agent].policy.critic for agent in agents]
            avg_state_dict = {}
            ...
            for critic in critics:
                ...
                for key in state_dict:
                    if state_dict[key].dtype == torch.long:
                        ...
                        avg_state_dict[key] += state_dict[key].float() / len(agents)
            # update critics
            for agent in agents:
                algorithms[agent].policy.critic.load_state_dict(avg_state_dict)
            # average actors
            actors = [algorithms[agent].policy.actor for agent in agents]
            avg_state_dict = {}
...
#     polar_thrust = list(((action + 1) / 2) * self.action_space)
#     mag, theta, phi = polar_thrust
#     thrust_r = mag * np.sin(theta) * np.cos(phi)
#     thrust_s = mag * np.cos(theta)                
#     thrust_w = mag * np.sin(theta) * np.sin(phi)
